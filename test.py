...
from fetch_txs import BitcoinClient
...
class TestFingerprinting(unittest.TestCase):
    def test_spending_types(self):
        tx = BitcoinClient.get_tx(
            "60849af6e56c2ad0facd601cc5014398210898a7e6d5b9280b54f6395349663a"
        )

        spending_types = get_spending_types(tx)
        assert spending_types == [
            "pubkeyhash",
            "witness_v0_keyhash",
        ] or spending_types == ["p2pkh", "v0_p2wpkh"]

        tx = BitcoinClient.get_tx(
            "E21c826797cdb41da79dc0f65026911ea5c5b59c8cf5d115f0f62b8cb9fc1b21"
        )
        ...
        assert spending_types == [
            ...
            "witness_v1_taproot",
            "witness_v0_keyhash",
        ] or spending_types == ["p2pkh", "v1_p2tr", "v0_p2wpkh"]
    ...
    def test_sending_types(self):
        ...
        assert sending_types == [
            "witness_v0_keyhash",
            "witness_v0_keyhash",
            "witness_v0_keyhash",
            "witness_v0_keyhash",
        ] or sending_types == ["v0_p2wpkh", "v0_p2wpkh", "v0_p2wpkh", "v0_p2wpkh"]

    def test_has_multi_type_vin(self):
        assert not has_multi_type_vin(
            BitcoinClient.get_tx(
                "5d857401648a667303cde43295bce1326e6329353eac3dddf15b151e701405e7"
            )
        )
        assert has_multi_type_vin(
            BitcoinClient.get_tx(
                "E21c826797cdb41da79dc0f65026911ea5c5b59c8cf5d115f0f62b8cb9fc1b21"
            )
        )
    ...
    def test_input_order(self):
        ...
        assert get_input_order(tx) == [
            InputSortingType.DESCENDING,
            InputSortingType.HISTORICAL,
        ]
    ...
    def test_get_change_index(self):
        ...
        assert (
            get_change_index(
                BitcoinClient.get_tx(
                    "d63aadc93aca05be5561d76888edf61e7f772b96fb1e43231111fe5fbcc4a601"
                )
            )
            == -2
        )
    ...
    def test_get_output_structure(self):
        ...
        assert get_output_structure(tx) == [
            OutputStructureType.DOUBLE,
            OutputStructureType.BIP69,
        ]

        tx = BitcoinClient.get_tx(
            "8b4e6d5fcab41058e9005f409aa505d95783b2cd52df1ec3c801cf06cac940f8"
        )
        assert get_output_structure(tx) == [OutputStructureType.SINGLE]

        tx = BitcoinClient.get_tx(
            "e5b278b6504297d0203a814a22239dad2b84742ec82a995c046dfef4e06fc5a4"
        )
        assert get_output_structure(tx) == [
            OutputStructureType.MULTI,
            OutputStructureType.CHANGE_LAST,
        ]
    ...
    def test_change_type_matched_inputs(self):
        assert (
            change_type_matched_inputs(
                BitcoinClient.get_tx(
                    "01d5bfed27b98cd049d5e3547e93a447df6cbfa1a1d64c33aff427bef8b3cec4"
                )
            )
            == 1
        )
    ...
    def test_signals_rbf(self):
        ...
        assert signals_rbf(
            BitcoinClient.get_tx(
                "Af39337032ec9d37aa91e41b152c1522cddc2070c08b06a1aaf5fe6ab6285a04"
            )
        )

    def test_address_reuse(self):
        assert address_reuse(
            BitcoinClient.get_tx(
                "43f901163b8c27567d365f56bb804bd74904bd78d58017905f3c36cac971d9b6"
            )
        )
        assert not address_reuse(
            BitcoinClient.get_tx(
                "1849118e584418ef9649e88e3b44a6ab6a6b06440fe7b1c51a1e76f16e72cefa"
            )
        )

    def test_bitcoin_core(self):
        wallet, _reasoning = detect_wallet(
            BitcoinClient.get_tx(
                "ba6e613d7894e81f369bdf1c77c57c772245643bef256f9df1e23bc0225b2e81"
            )
        )
    ...
    def test_trezor(self):
        ...
        wallet, _reasoning = detect_wallet(
            BitcoinClient.get_tx(
                "87670b12778d17c759db459479d66acfd1c4d444094270991d8e1de09a56cc7c"
            )
        )
        assert wallet == {Wallets.TREZOR}
    ...
    def test_ledger(self):
        ...
        wallet, _reasoning = detect_wallet(
            BitcoinClient.get_tx(
                "047b1779fceb28852d890fd36bbc0481ed7aa8eb8b73fc1ab19d7707780c041d"
            )
        )
        assert wallet == {Wallets.LEDGER}
